var margin_0101 = {top: 50, right: 50, bottom: 50, left: 60},
...
d3.tsv("../../data/01_hozzaferhetoseg_es_megfizethetoseg/01_01_user_input.tsv",  function(error, data_0101) {
	...
	svg_0101.append("g")
        ...
        .attr("y",  margin_0101.bottom-20)
		.style("text-anchor", "middle")
	    .style("fill", "#000000")
		.style("font-size", "12px")
		.text("Jövedelmi decilis");
	...
	svg_0101.append("g")
		...
		.attr("y", 0 - (margin_0101.left-10))
		.attr("x", 0  - (userInputHeight/2) )
		.style("text-anchor", "middle")
		.style("fill", "#000000")
	    .style("font-size", "12px")
...
function myFunction() {
	...
	d3.tsv("../../data/01_hozzaferhetoseg_es_megfizethetoseg/01_01_user_input.tsv",  function(error, data_0101) {
		...
		svg_0101.selectAll("text.label")
			...
			.style("text-anchor", "middle")
			.text(dec + ". jövedelmi decilisbe tartozik,")
...
console.log(data_0101.filter(function(d) { return d["Decilis"] == dec; }))
		
		svg_0101.selectAll("line.arrow")
			.data(data_0101.filter(function(d) { return d["Decilis"] == dec; }))
			.enter().append("line")
			.attr("class", "arrow")
			.attr("stroke", "#606060")
			.attr("x1", function (d) {
			...
			.attr("x2", function (d) {
			var totalband = userInputWidth/10;
			var pad_in = totalband * 0.1;
			var myband = (userInputWidth - (pad_in * 10))/10;
			return ((dec) * myband + (dec-1) * pad_in) - (myband*0.45);
		})
			.attr("y1", function (d) {
			return 65;
		})
