...
function BarChart() {
	...
	data.forEach(function(d) {
	  ...
	  color.domain(d3.keys(data[0]).filter(function(key) { return key !== "Régió"; }));
...
data.sort(function (a,b) {return d3.ascending(a.Régió, b.Régió);});
data.sort(function(a, b) { return b.total - a.total; });

	
  x.domain(data.map(function(d) { return d.Régió; }));
  y.domain([0, 1]);
...
function restorePlot(d) {
    ...
    d3.selectAll(".class" + class_keep)
      .attr("x", function(d) { return x(d.myregion); })
      .transition()
      .duration(1000)
      .delay(function () {
